  ...
  <div class="msg">
    ...
    <div class="msg-list">
      ...
      <transition-group name="show">
        <div v-for="(item, index) of (isOnRead ? msg.hasnot_read_messages : msg.has_read_messages)" :key="item.id+index"
             class="msg-item">
          <div class="msg-body">
            <div class="msg-name">
                <span @click="view">
                  <router-link
                    :to="{name: 'user', params: {name: item.author.loginname}}">{{ item.author.loginname }}</router-link>
                </span> 的回复：
  ...
  .msg {
    ...
    .msg-header {
      ...
      i.icon-back {
        ...
        width: 38px;
        height: 38px;
        background: url('./icon-back.svg') no-repeat;
